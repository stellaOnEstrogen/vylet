	...
	async execute(interaction: CommandInteraction): Promise<any> {
		...
		if (subcommand === 'list') {
			...
			if (category === 'user') {
				if (
					!user ||
					!user.market_items ||
					user.market_items === 'null' ||
					!user.market_items.length
				) {
				...
				const items =
					Array.isArray(user.market_items) ?
						user.market_items
					:	JSON.parse(user.market_items);
				let page = 1;
				...
				collector.on('collect', async (i) => {
					if (i.customId === 'next_page') {
						if (page * MARKET_PAGE_SIZE >= items.length) {
							collector.stop();
							return i.update({
								content: 'You have reached the end of your items',
								components: [],
							});
						}
						page++;
						const newEmbed = createItemEmbed(
							'Your Items',
							'Items in your inventory:',
							items,
							page,
			...
			if (category === 'all') {
				...
				const embed = createItemEmbed(
					'Market Items',
					'Here are all the items available in the market:',
					marketItems,
					page,
					MARKET_PAGE_SIZE,
				);
				...
				collector.on('collect', async (i) => {
					if (i.customId === 'next_page') {
						if (page * MARKET_PAGE_SIZE >= marketItems.length) {
							collector.stop();
							return i.update({
								content: 'You have reached the end of the market items',
								components: [],
							});
						}
						page++;
		...
		if (subcommand === 'buy') {
			...
			const itemName = interaction.options
				...
				.getString('item')
				?.replace(/ /g, '_');
			const itemData = await db.queryRaw(
			...
			await db.queryRaw('UPDATE Users SET market_items = ? WHERE id = ?;', [
			...
			await db.queryRaw(
				'UPDATE Market SET demand = demand + 1, supply = supply - 1 WHERE item_id = ?;',
				[item.item_id],
			);
		...
		if (subcommand === 'sell') {
			...
			// Add item to market
			await db.queryRaw(
				'INSERT INTO Market (item_name, item_id, base_price, current_price, supply) VALUES (?, ?, ?, ?, ?)',
